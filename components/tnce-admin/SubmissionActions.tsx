...
type Props = {
  ...
  productionRecord: TNCEPublishRecord;
...
export default function SubmissionActions({
  ...
  async function submitStatusAction(
    ...
    if ((action === "needs-info" || action === "reject") && !cleanedNotes) {
    ...
    try {
      ...
      const response = await fetch("/api/tnce/admin/action", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || !data.ok) {
        throw new Error(data.error || "TNCE action failed.");
      }

      onStatusChange?.(payload.submissionId, status, cleanedNotes);
    ...
    try {
      const response = await fetch("/api/tnce/admin/publish", {
        ...
        body: JSON.stringify({
          ...
          organizedImages: organizedImages.map((image) => ({
            id: image.id,
            url: image.url,
            role: image.role,
      ...
      <section className="rounded-2xl border border-neutral-800 bg-neutral-950 p-4">
        ...
        {activeAction && (
          <div className="mt-4 rounded-xl border border-[#d4af37]/50 bg-[#181300] p-3">
            ...
            {(activeAction === "needs-info" || activeAction === "reject") &&
              !reviewNotes.trim() && (
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (activeAction === "reset-pending") {
                    submitStatusAction("reset-pending", "Pending Review");
                  }
                }}
